...
1. **Start / входная нода чата** (AgentFlow V2)  
   ...
   - В **Flow State** инициализируется ключ `current_topics` (значение по умолчанию: пустая строка `""`).

2. **Retriever (RAG)** — одна нода AgentFlow V2  
   - Ищет по markdown‑базе клиники (Document Store, при необходимости Chroma).
   - **Output Format:** обязательно **Text with Metadata**, чтобы в Custom Function были метаданные источника (имя файла) для группировки по теме.
...
3. **Custom Function** — извлечение доступных секций  
   - Один вход: выход Retriever (в конфиге ноды: Input Variable `retrieverOutput` = `{{ RetrieverNodeId.output }}`).
   - Логика: разбор чанков, извлечение строк с заголовками `###` (regex), исключение раздела «Коротко», группировка по теме из метаданных (source/fileName).
   - Возвращает **строку** — список заголовков через запятую (например: `Кому подходит, Ограничения, Стоимость, Что входит`), без JSON, чтобы избежать двойной упаковки в state.
   - **Update Flow State:** вывод ноды записать в ключ `current_topics`.
   - Код для копирования в UI Flowise: см. файл `flowise/custom-function-extract-sections.js` в репозитории.

4. **LLM‑нода с системным промптом + Structured Output** (AgentFlow V2)  
   - В системный промпт подставляется `{{ $flow.state.current_topics }}` (список доступных подразделов по текущему запросу); см. правила в `prompt.md` (блок про RAG и current_topics).
...
Порядок выполнения: Start → Retriever → Custom Function → LLM.

### 2.2. Ограничения P0 по JSON‑ответу

В P0 используется **минимальная схема Structured Output** с **плоскими ключами** (как в настройке LLM‑ноды и в `prompt.md` §14):

- `answer` — текст ответа пользователю
- `ui_ctaIntent` — намерение по CTA (`"booking"` или `"none"`)
- `meta_stage` — стадия воронки (`discovery` | `interest` | `ready` | …)
- `meta_confidence` — уверенность модели в оценке (число)
- `flags_emotional` — эмоциональное состояние пользователя (boolean)
- `leadIntent` — режим записи для виджета: `none` | `awaiting_name` | `awaiting_phone` | `complete`
- `meta_shouldHandoff` — рекомендация передать диалог администратору (boolean)

Полная и расширенная схема (P1+) описана в `cta.md` и в соответствующем разделе `prompt.md`.  
При расхождениях по полям или названиям **источником истины являются `prompt.md` (§14) и `cta.md`**.

### 2.3. RAG и список доступных секций (current_topics)

Чтобы бот не предлагал «рассказать подробнее» по темам, которых нет в извлечённых чанках, между Retriever и LLM добавлена нода **Custom Function**:

- **Назначение:** по выходу Retriever детерминированно (regex) извлечь все заголовки `###` из текста чанков, исключить «Коротко», сгруппировать по теме (из метаданных: source/fileName) и записать результат в Flow State под ключом `current_topics`.
- **Формат в state:** простая строка — список подразделов через запятую (например: `Кому подходит, Ограничения, Стоимость, Что входит`). В промпте LLM подставляется как `{{ $flow.state.current_topics }}`.
- **Требования к Retriever:** Output Format — **Text with Metadata**; при загрузке документов в Document Store должно сохраняться имя файла (или путь) в метаданных чанков, иначе группировка по теме будет сводиться к одной теме «Контекст».
- **Код Custom Function:** файл `flowise/custom-function-extract-sections.js` в репозитории. В UI Flowise вставить в поле Javascript Function ноды, Input Variable: `retrieverOutput` = `{{ <id ноды Retriever>.output }}`, Update Flow State: ключ `current_topics` = вывод этой ноды.

Если ретривер ничего не вернул, Custom Function возвращает пустую строку; в промпте задано правило: в этом случае не импровизировать, сообщить, что по запросу информации в базе нет.
